...
import kr.co.t_woori.good_donation_admin.databinding.ProjectSignupActivityBinding;
...
/**
 * Created by rladn on 2017-08-23.
 */

public class ProjectSignupActivity extends AppCompatActivity {
    private ProjectSignupActivityBinding binding;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = DataBindingUtil.setContentView(this, R.layout.project_signup_activity);
    ...
    private boolean isAllDataValid(){
        if(!isFilled(binding.nameEdt)){
            Utilities.showToast(this, "프로젝트명을 입력해주세요.");
        }else if(!isFilled(binding.goalEdt)){
            Utilities.showToast(this, "목표금액을 입력해주세요.");
        }else if(!isFilled(binding.introEdt)){
            Utilities.showToast(this, "프로젝트 한줄 소개를 입력해주세요.");
        }else if(!isFilled(binding.appreciationEdt)){
            Utilities.showToast(this, "프로젝트 감사 문구를 입력해주세요.");
        }else{
            return true;
        }
        return false;
    ...
    private HashMap<String, String> getCharityInfo(){
        ...
        charityInfo.put("goal", binding.goalEdt.getText().toString().trim());
    ...
    private void projectSignup(HashMap<String, String> projectInfo){
        ServerCommunicator serverCommunicator = new ServerCommunicator(
                this, APICreator.create(MasterAPIService.class).charitySignup(projectInfo)
        ) {
            @Override
            protected void onSuccess(HashMap<String, Object> results) {
                final String idNum = (String)results.get("idNum");
                AlertDialog.Builder builder = new AlertDialog.Builder(ProjectSignupActivity.this);
                builder.setTitle("기부프로젝트 등록 완료")
                        .setMessage("기부프로젝트 이미지를 바로 등록하시겠습니까?")
                        .setPositiveButton("예", new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Intent intent = new Intent(ProjectSignupActivity.this, ImageUploader.class);
